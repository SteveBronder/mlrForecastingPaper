...
\singlespacing
<<makeEts, cache = TRUE>>=
ets.mod =makeLearner("fcregr.ets",
                     damped = TRUE,
...
# Plot the overlayed series
dev.copy(png,'plot_ets_train.png')
...
<<loadTuneets2, cache = TRUE, echo = FALSE, results = 'hide'>>=
...
plotHyperParsEffect(ets.hyp, x= "damped", y = "mase.test.mean",
                     plot.type = "scatter",
                     partial.dep.learn = "regr.randomForest")
...
q = q + theme_tufte()
ggsave("./ets_dep_plot_max_q.png",q)
p = plotHyperParsEffect(ets.hyp, x= "damped", y = "mase.test.mean",
                     plot.type = "scatter",
                     partial.dep.learn = "regr.randomForest")

p = p + theme_tufte()
ggsave("./ets_dep_plot_max_p.png",p)
@
...
For comparison, ETS is run in \pkg{forecast} with it's base parameters
...
pred.ets.dat = zoo(cbind(climate.pred$data, as.data.frame(forecast.ets$mean)), order.by = index(m4.test))
colnames(pred.ets.dat)[3] <- "forecast_ets" 
col_names = colnames(pred.ets.dat)
dev.copy(png,'plot_ets_vs_forecast.png')
# Set a color scheme:
tsRainbow <- rainbow(ncol(pred.ets.dat))
# Plot the overlayed series
