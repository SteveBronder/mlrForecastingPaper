...
plot(m4Train1, main = "Weekly Climate Data")
...
<<fin_task, eval = TRUE>>=
library(mlr)
Fin.task1 = makeForecastRegrTask(id = "M4 Climate Data",
                                 data = m4Train1,
                                 target = "target_var",
                                 frequency = 52L)
Fin.task1
...
<<makeArima>>=
arm = makeLearner("fcregr.Arima", order = c(2L,1L,1L), h = 35L, include.mean = FALSE)
arm
@
...
resampDesc = makeResampleDesc("GrowingCV", horizon = 35L,
                               initialWindow = 400L,
                               size = nrow(getTaskData(Fin.task1)), skip = 2L)
resampDesc
@
