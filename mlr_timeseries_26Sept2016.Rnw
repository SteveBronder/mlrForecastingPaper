...
<<makeArima>>=
arm = makeLearner("fcregr.Arima", order = c(2L,0L,1L), h = 328L, include.mean = FALSE)
arm
@
...
<<makeArimaTrain>>=
trainArima = train(arm, Fin.task1 )
trainArima
@
...
resampDesc = makeResampleDesc("GrowingCV", horizon = 328L,
                               initialWindow = 6000L,
                               size = nrow(getTaskData(Fin.task1)), skip = 100L)
resampDesc
@
...
#
res = tuneParams("fcregr.Arima", task = Fin.task1, resampling = resampDesc, par.set = parSet,
...
<<predbestArima, eval = FALSE>>=
predict(m, task = Fin.task1, newdata = m4Test1)
@
