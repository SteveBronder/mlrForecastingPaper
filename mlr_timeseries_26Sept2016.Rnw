...
euro.classif.train = euro.classif.data[1:1850,,drop = FALSE]
euro.classif.test  = euro.classif.data[1851:1853,,drop = FALSE]
...
classif.fc = forecast(classif.train, h=2, newdata = euro.classif.test)
...
<<ensembleModel1, cache = TRUE, eval = FALSE>>=
...
<<ensembleModel2, cache = TRUE, eval = FALSE>>=
...
ps = makeParamSet(
  ...
  makeDiscreteParam(id = "fcregr.garch.model", values = c("csGARCH")),
...
<<ensembleModel4, cache = TRUE, eval = FALSE>>=
# get hyper params
...
<<multiforecast, eval = FALSE>>=
multfore.task = makeMultiForecastRegrTask(id = "bigvar", data = eu.train, target = "FTSE")
...
ps = makeParamSet(
  ...
  makeNumericVectorParam("mfcregr.BigVAR.gran", len = 2L, lower = 25, upper = 30),
